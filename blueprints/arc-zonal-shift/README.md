...
This blueprint uses the `default` Karpenter `NodePool` and `EC2NodeClass`. Set your environment variables:
...
Deploy all three workloads representing different configurations:
...
kubectl get nodes -L topology.kubernetes.io/zone,karpenter.sh/nodepool -l karpenter.sh/nodepool=default
```
...
```sh
kubectl get nodes -L topology.kubernetes.io/zone -l karpenter.sh/nodepool=default
```
...
The stateful workload's pod is bound to an EBS volume in one AZ. If that AZ is under the shift:

```sh
kubectl get pods -l app=web-app-stateful -o wide
```

The pod remains `Pending` because its volume is physically in the impaired AZ and cannot move. Karpenter will not attempt to launch a node in the impaired AZ for this pod. This is a known limitation of ARC zonal shift with zonal storage like EBS. See the [EKS zonal shift documentation](https://docs.aws.amazon.com/eks/latest/userguide/zone-shift.html) for more details.
